...
def test_api_request_refreshes_when_token_is_expired():
    c = Client()
    c.oauth2_token = OAuth2Token(access_token="expired-token", expires_at=0)
...
def test_non_api_request_skips_auth():
    c = Client()
    c.oauth2_token = OAuth2Token(access_token="ok", expires_at=int(time.time()) + 3600)
...
def test_dict_token_does_not_leak_into_header():
    c = Client()
    c.oauth2_token = {"access_token": "stale", "expires_at": 0}

    resp = c.request("GET", "/me", api=True)

    assert "stale" not in resp["headers"].get("Authorization", "")
    assert isinstance(c.oauth2_token, OAuth2Token)